...
class Robot:
    ...
    def process_transition(self, state, action, next_state, money_remaining):
        if(float(money_remaining)<=0):
            print(f"transition - catch:{money_remaining}")
        if np.linalg.norm(state - next_state) <= 5.0:
            self.stuck = True
        ...
        if self.plan_index == len(self.planned_actions) - 1 or np.linalg.norm(next_state - self.goal_state) < 3.0:
    ...
    def get_next_action_type(self, state, money_remaining):
        if(float(money_remaining)<=0):
            print(f"action type - catch:{money_remaining}")
        ...
        if False:
            return 'demo'
        if np.all(self.planned_actions == 0) or np.linalg.norm(state-self.goal_state) < 3.0:
            if float(money_remaining) > float(constants.COST_PER_RESET):
                #print("reset")
                self.planned_actions = np.zeros((constants.DEMOS_CEM_PATH_LENGTH, 2))
    ...
    def get_next_action_training(self, state, money_remaining):
        if(float(money_remaining)<=0):
            print(f"next action traning - catch:{money_remaining}")
